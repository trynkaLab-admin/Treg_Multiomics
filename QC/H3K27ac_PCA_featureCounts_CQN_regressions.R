...
input=c("ChM_K27ac_Peak_featureCounts")
file=paste(input,".txt",sep="")
data <- read.table(file=file, row.names=1, header=TRUE, sep="\t")
counts_matrix <- data[rowSums(data) > 0, ]
gene_metadata <- read.table("~/Desktop/Tregs/LRRC32/LRRC32_plots/K27ac_Peak_MetaData.txt", header=TRUE, sep="\t")

ggplot(gene_metadata, aes(x=length))+
  geom_histogram(binwidth=100,color="#7bbcb5", alpha = 0.9)+ 
  geom_vline(xintercept = median(gene_metadata$length),color="#7bbcb5")+
...
annotate("text", x = median(gene_metadata$length)+20000, y = 750, label = paste("Median peak length = ", round(median(gene_metadata$length),digits=2),sep=""))

ggsave(paste(input,"_PeakLength_Paper.pdf",sep=""), height=5, width=5, dpi=300, units = c("cm"))

###Number of reads per sample and peak
breaks=c(seq(0,40000,100),max(rowMeans(data)))
ggplot(data,aes(x=rowMeans(data)))+
  geom_histogram(breaks=breaks, color="#7bbcb5", fill="#7bbcb5", alpha = 0.2)+
  coord_cartesian(xlim = c(0, 6000)) +
  geom_vline(xintercept = median(rowMeans(data)),color="#7bbcb5")+
  theme(strip.background=element_blank(),panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),text=element_text(size=12))+
...
ggsave(paste("MedianReads_",input,".pdf"), height=5, width=5, dpi=300, units = c("cm"))
...
counts_matrix_cqn2 <- read.table(file=paste(input,"_NoBatchCorrection_CQN.txt",sep=""), sep="\t", header = TRUE)

##############################################
#PCA without batch correction

data.pca.nocor <- prcomp(t(counts_matrix_cqn2), center = TRUE, scale = TRUE)
write.table(data.frame(data.pca.nocor$x[,1:35]), file=paste(input,"_noBatchCorrection_CQN.PCA",sep=""), sep = "\t", row.names=T)
influence.nocor <- abs(data.pca.nocor$rotation)
SNPinfluence.nocor <- as.matrix(sweep(influence.nocor, 2, colSums(influence.nocor), "/"))
write.table(SNPinfluence.nocor, file=paste(input,"_NoBatchCorrection_CQN.BinContribution",sep=""), sep = "\t", row.names=T)
png(file=paste("PCs_Variance_NoBatchCorrection_CQN_",input,".png",sep=""), height=20, width=40, unit="cm", res=300)
...
ggsave(paste("PC1_PC2_",input,"_ColoredBySex_NoBatchCor_CQN.png"), height=10, width=10, dpi=300)
...
ggsave(paste("PC1_PC2_",input,"_ColoredByBatch_NoBatchCor_CQN.png"), height=10, width=10, dpi=300)

ggplot(Forplotting.nocor,aes(scores, x=PC1, y=PC2, colour=as.factor(Forplotting.nocor$Plate)))+ 
  geom_point() + labs(color='Plate') +geom_text(aes(label= Forplotting.nocor$Row.names),hjust=0, vjust=0, size = 4)
ggsave(paste("PC1_PC2_",input,"_ColoredByPlate_NoBatchCor_CQN.png"), height=10, width=10, dpi=300)
...
ggsave(paste(input,"_PerCentVariance_Batch_NoBatchCor_CQN.png",sep=""), height=10, width=10, dpi=300)

qplot(data=Forplotting.nocor, PC1, PC2, color= Sex,main="PC1 vs PC2", size =I(4))+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.nocor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.nocor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_line(colour="gray50"), panel.grid.major.y = element_line(colour="gray50"), panel.grid.minor = element_blank(),panel.background = element_blank(),text=element_text(size=20))+scale_color_manual(values=c("blue","purple","darkorange","red","black","green","orange"),guide=guide_legend(title = "Gender"))
ggsave(paste(input,"_PerCentVariance_Sex_NoBatchCor_CQN.png",sep=""), height=10, width=10, dpi=300)

qplot(data=Forplotting.nocor, PC1, PC2, color= Plate,main="PC1 vs PC2", size =I(4))+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.nocor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.nocor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_line(colour="gray50"), panel.grid.major.y = element_line(colour="gray50"), panel.grid.minor = element_blank(),panel.background = element_blank(),text=element_text(size=20))+scale_color_manual(values=c("blue","purple","darkorange","red","black","green","orange"),guide=guide_legend(title = "Plate"))
ggsave(paste(input,"_PerCentVariance_Plate_NoBatchCor_CQN.png",sep=""), height=10, width=10, dpi=300)
...
aov5 = aov(Forplotting.nocor$PC1 ~ Forplotting.nocor$Plate)
summary(aov5)
aov6 = aov(Forplotting.nocor$PC2 ~ Forplotting.nocor$Plate)
summary(aov6)
...
regressedPCs <- regressPrinicpalComponents(data_matrix = counts_matrix_cqn2, n_pcs = 22)
###Save CQN counts with PCs regressed

write.table(regressedPCs, file=paste(input,"_NoBatchCor_CQN_22PCsRegressed.txt",
                                     sep=""), sep = "\t", row.names=T)


data.pcaregressed <- prcomp(t(regressedPCs), center = TRUE, scale = TRUE)
influence <- abs(data.pcaregressed$rotation)
summary(data.pcaregressed)

#PLOT

scores_2 <- data.frame(data.pcaregressed$x[,1:22])
Forplotting2 <- merge(scores_2, covariates, by="row.names")
percentVar.PCcor <- round(100*data.pcaregressed$sdev^2/sum(data.pcaregressed$sdev^2),1)

qplot(data=Forplotting2, PC1, PC2, color= Month_batch,main="PC1 vs PC2", size =I(4))+
  labs(x=paste0("PC1,VarExp:",round(percentVar.PCcor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.PCcor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_line(colour="gray50"), panel.grid.major.y = element_line(colour="gray50"), panel.grid.minor = element_blank(),panel.background = element_blank(),text=element_text(size=20))+scale_color_manual(values=c("blue","purple","darkorange","red","black","green","orange"),guide=guide_legend(title = "Batch"))
...
qplot(data=Forplotting2, PC1, PC2, color= Plate,main="PC1 vs PC2", size =I(4))+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.PCcor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.PCcor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_line(colour="gray50"), panel.grid.major.y = element_line(colour="gray50"), panel.grid.minor = element_blank(),panel.background = element_blank(),text=element_text(size=20))+scale_color_manual(values=c("blue","purple","darkorange","red","black","green","orange"),guide=guide_legend(title = "Plate"))
...
summary(aov16)
aov17 = aov(Forplotting2$PC1 ~ Forplotting2$Plate)
summary(aov17)
aov18 = aov(Forplotting2$PC2 ~ Forplotting2$Plate)
summary(aov18)
aov19 = aov(Forplotting2$PC3 ~ Forplotting2$Month_batch)
summary(aov19)
aov20 = aov(Forplotting2$PC3 ~ Forplotting2$Sex)
summary(aov20)
aov21 = aov(Forplotting2$PC3 ~ Forplotting2$Plate)
summary(aov21)
aov22 = aov(Forplotting2$PC4 ~ Forplotting2$Month_batch)
summary(aov22)
aov23 = aov(Forplotting2$PC4 ~ Forplotting2$Sex)
summary(aov23)
aov24 = aov(Forplotting2$PC4 ~ Forplotting2$Plate)
summary(aov24)
aov25 = aov(Forplotting2$PC5 ~ Forplotting2$Month_batch)
summary(aov25)
aov26 = aov(Forplotting2$PC5 ~ Forplotting2$Sex)
summary(aov26)
aov27 = aov(Forplotting2$PC5 ~ Forplotting2$Plate)
summary(aov27)
aov28 = aov(Forplotting2$PC6 ~ Forplotting2$Month_batch)
summary(aov28)
aov29 = aov(Forplotting2$PC6 ~ Forplotting2$Sex)
summary(aov29)
aov30 = aov(Forplotting2$PC6 ~ Forplotting2$Plate)
summary(aov30)
aov31 = aov(Forplotting2$PC7 ~ Forplotting2$Month_batch)
summary(aov31)
aov32 = aov(Forplotting2$PC7 ~ Forplotting2$Sex)
summary(aov32)
aov33 = aov(Forplotting2$PC7 ~ Forplotting2$Plate)
summary(aov33)
aov34 = aov(Forplotting2$PC8 ~ Forplotting2$Month_batch)
summary(aov34)
aov35 = aov(Forplotting2$PC8 ~ Forplotting2$Sex)
summary(aov35)
aov36 = aov(Forplotting2$PC8 ~ Forplotting2$Plate)
summary(aov36)
aov37 = aov(Forplotting2$PC9 ~ Forplotting2$Month_batch)
summary(aov37)
aov38 = aov(Forplotting2$PC9 ~ Forplotting2$Sex)
summary(aov38)
aov39 = aov(Forplotting2$PC9 ~ Forplotting2$Plate)
summary(aov39)
aov40 = aov(Forplotting2$PC10 ~ Forplotting2$Month_batch)
summary(aov40)
aov41 = aov(Forplotting2$PC10 ~ Forplotting2$Sex)
summary(aov41)
aov42 = aov(Forplotting2$PC10 ~ Forplotting2$Plate)
summary(aov42)

#Paper figures
ggplot(Forplotting.nocor, aes(x=PC1, y=PC2)) + 
  geom_point(color="#7bbcb5", alpha = 0.5, size = 3)+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.nocor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.nocor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),text=element_text(size=12))
...
ggplot(Forplotting2, aes(x=PC1, y=PC2)) + 
  geom_point(color="#7bbcb5", alpha = 0.9, size = 3)+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.PCcor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.PCcor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),text=element_text(size=12))
