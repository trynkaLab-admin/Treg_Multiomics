...
input=c("ChM_K4me3_Peak_featureCounts")
file=paste(input,".txt",sep="")
data <- read.table(file=file, row.names=1, header=TRUE, sep="\t")
counts_matrix <- data[rowSums(data) > 0, ]
gene_metadata <- read.table("K4me3_Peak_MetaData.txt", header=TRUE, sep="\t")

ggplot(gene_metadata, aes(x=length))+
  geom_histogram(binwidth=100,color="#ee7566", alpha = 0.9)+ 
  geom_vline(xintercept = median(gene_metadata$length),color="#ee7566")+
...
ggplot(data,aes(x=rowMeans(data)))+
  geom_histogram(breaks=breaks, color="#ee7566", fill="#ee7566", alpha = 0.2)+
  coord_cartesian(xlim = c(0, 6000)) +
  geom_vline(xintercept = median(rowMeans(data)))+
  theme(strip.background=element_blank(),panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),text=element_text(size=12))+
...
regressedPCs <- regressPrinicpalComponents(data_matrix = counts_matrix_cqn2, n_pcs = 33)
###Save CQN counts with PCs regressed

write.table(regressedPCs, file=paste(input,"_NoBatchCor_CQN_33PCsRegressed.txt",
                                     sep=""), sep = "\t", row.names=T)
...
write.table(Forplotting2, file = "Test.txt", quote = FALSE, row.names = FALSE)
...
ggplot(Forplotting.nocor, aes(x=PC1, y=PC2)) + 
  geom_point(color="#ee7566", alpha = 0.5, size = 3)+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.nocor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.nocor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),text=element_text(size=12))
...
ggplot(Forplotting2, aes(x=PC1, y=PC2)) + 
  geom_point(color="#ee7566", alpha = 0.9, size = 3)+ 
  labs(x=paste0("PC1,VarExp:",round(percentVar.PCcor[1],4)), y=paste0("PC2,VarExp:",round(percentVar.PCcor[2],4)))+ 
  theme(strip.background=element_blank(),panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),text=element_text(size=12))
